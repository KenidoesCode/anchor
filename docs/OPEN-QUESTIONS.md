...
- **Q-P1-8 — Concurrent deployments / overlap rule.** Can one officer be posted to two sites at once? Are touching intervals (one ends the day the next starts) an overlap? *KK ratified the current behaviour — any overlap is a hard block, intervals inclusive (ADR-0014) — but this is a domain fact only Greensafe can confirm; flagged in code.* *(SPEC-REVIEW A6)* — **[RATIFIED by KK; awaiting Greensafe confirmation]**
- **Q-P1-14 — Requirement version resolved as-of when?** The gate resolves the in-force requirement version as-of **server-clock today**, not the deployment **start date** — so a posting starting in three weeks is validated against today's requirements. KK's instinct is that a future posting should use the requirements in force at its start. Behaviour left unchanged pending this answer; flagged in code (ADR-0015). — **[OPEN — behaviour frozen pending answer]**
